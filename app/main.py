...
def create_application() -> FastAPI:
    ...
    async def validation_exception_handler(request: Request, exc: RequestValidationError):
        ...
        logger.error(f"Body: {body.decode(errors='replace')}")
    ...
    try:
        ...
        logger.warning(f"Could not ensure media directories at {media_dir}: {e}")
    ...
    logger.debug(f"Serving media from {media_dir}")
    ...
    async def get_logo():
        from fastapi.responses import FileResponse
        return FileResponse(os.path.join(project_root, "app", "templates", "logo.png"))
