...
async def send_fcm_notification(
    ...
    try:
        ...
        # Мы используем ТОЛЬКО Data-сообщения для Android, чтобы избежать дублирования уведомлений.
        # Если в сообщении есть объект notification, Android сам показывает уведомление,
        # и Notifee (вызываемый из background handler) показывает второе.
        # Оставляем только Data, чтобы Notifee отобразил одно кастомное уведомление с аватаркой.
