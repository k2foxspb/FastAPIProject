...
def upgrade() -> None:
    ...
    if is_postgres:
        op.add_column('products', sa.Column('tsv', sa.dialects.postgresql.TSVECTOR(), sa.Computed("setweight(to_tsvector('english', coalesce(name, '')), 'A') || setweight(to_tsvector('english', coalesce(description, '')), 'B')", persisted=True), nullable=False))
